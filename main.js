// Main.js - Endless Car Simulator with Ford Focus on Interstate Road
...
// Create a single road segment with lane markings
...
// Position wheels (front left, front right, rear left, rear right)
...
function animate() {
  ...
  // Update steering (steering only affects if moving forward or backward)
  if (turnLeft && Math.abs(speed) > 0.01) {
    car.rotation.y += steeringAngle * (speed / maxSpeed);
  }
  if (turnRight && Math.abs(speed) > 0.01) {
    car.rotation.y -= steeringAngle * (speed / maxSpeed);
  }
  ...
  if (car.position.z < firstSegment.road.position.z - segmentLength) {
    // Move the first segment to the back
    ...
    firstSegment.markings.children.forEach(marking => {
      marking.position.z = marking.position.z - (lastSegment.road.position.z - firstSegment.road.position.z);
    });
  ...
  // Smooth camera follow behind the car
